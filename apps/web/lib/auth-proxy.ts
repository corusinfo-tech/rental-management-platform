...
function setRefreshCookies(
  ...
  // Domain is deliberately omitted so the refresh token remains host-only.
  response.cookies.set({
    name: REFRESH_COOKIE,
    value: refreshToken,
    httpOnly: true,
...
async function parseBody(response: Response): Promise<unknown> {
  ...
  if (!text)
    return {
      success: false,
      error: { message: 'Authentication service returned an empty response' },
    };
  try {
    ...
    return {
      success: false,
      error: { message: 'Authentication service returned an invalid response' },
    };
  }
}
...
export async function handleAuthProxy(
  ...
  if (!hasAllowedBrowserOrigin(request))
    return NextResponse.json(
      { success: false, error: { message: 'Cross-site authentication request rejected' } },
      { status: 403 },
    );
  ...
  if (path === 'refresh') {
    ...
    if (!body || typeof body.identifier !== 'string' || typeof body.password !== 'string') {
      return NextResponse.json(
        { success: false, error: { message: 'Invalid login request' } },
        { status: 400 },
      );
    }
  ...
  try {
    backendResponse = await fetch(backendUrl(path), {
    ...
    return NextResponse.json(
      {
        success: false,
        error: { message: 'Authentication service is unavailable. Please try again.' },
      },
      { status: 503 },
    );
  }
  ...
  if (
    ...
    backendResponse.ok &&
    isSuccessEnvelope(payload)
  ) {
    const { refreshToken, ...publicTokens } = payload.data;
    const response = NextResponse.json(
      { ...payload, data: publicTokens },
      { status: backendResponse.status },
    );
    setRefreshCookies(
      response,
      refreshToken,
      path === 'login' ? rememberMe : request.cookies.get(REMEMBER_COOKIE)?.value === '1',
      cookiePath,
    );
    return response;
  }
  ...
  if (path === 'refresh' || path === 'logout' || path === 'logout-all')
    clearRefreshCookies(response);
  return response;
}
