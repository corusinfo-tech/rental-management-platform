...
export class OrganizationSettingsRepository {
  ...
  async findOrCreateDefaults(organizationId: string, transaction: Prisma.TransactionClient) {
    ...
    await transaction.organizationSettings.createMany({
      data: [{ id: randomUUID(), organizationId, timezone: organization.timezone, currency: organization.currency, country: organization.country }],
      skipDuplicates: true,
    });
    return transaction.organizationSettings.findUnique({ where: { organizationId } });
  }
  ...
  settingsAccess(userId: string, organizationId: string, permissionCode: string, transaction: Prisma.TransactionClient) {
    return transaction.organizationMembership.findFirst({
      where: { organizationId, status: MembershipStatus.ACTIVE, deletedAt: null, organization: { deletedAt: null }, person: { user: { id: userId, deletedAt: null } }, roles: { some: { role: { code: { notIn: ['SUPER_ADMIN', 'OWNER', 'LANDLORD'] }, deletedAt: null, permissions: { some: { permission: { code: permissionCode, deletedAt: null } } } } } } },
    });
  }
