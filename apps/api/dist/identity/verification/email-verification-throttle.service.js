...
exports.EmailVerificationThrottleService = void 0;
...
let EmailVerificationThrottleService = class EmailVerificationThrottleService {
    config;
    prisma;
    redis;
    ...
    async enforce(email, request) {
        ...
        const emailFingerprint = this.fingerprint((0, normalization_1.normalizeEmail)(email));
        ...
        if (ipCount <= settings.requestIpLimit && emailCount <= settings.requestEmailLimit)
            return;
        ...
        throw new common_1.HttpException('Too many verification requests. Please try again later.', common_1.HttpStatus.TOO_MANY_REQUESTS);
    }
    async increment(key, ttl) {
        const value = await this.redis.eval("local count = redis.call('INCR', KEYS[1]); if count == 1 then redis.call('EXPIRE', KEYS[1], ARGV[1]); end; return count;", 1, key, ttl);
        return Number(value);
    }
    fingerprint(value) {
...
exports.EmailVerificationThrottleService = EmailVerificationThrottleService;
exports.EmailVerificationThrottleService = EmailVerificationThrottleService = __decorate([
