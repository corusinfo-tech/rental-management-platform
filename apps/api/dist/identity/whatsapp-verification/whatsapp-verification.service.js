...
let WhatsAppVerificationService = class WhatsAppVerificationService {
    repository;
    verificationEngine;
    ...
    async request(mobile, correlationId) {
        ...
        await this.verificationEngine.resendVerification({ userId: user.id, organizationId: organizationId ?? null, channel: client_1.VerificationChannel.WHATSAPP, purpose: client_1.VerificationPurpose.WHATSAPP_OTP, correlationId });
        return ACCEPTED;
    }
    async confirm(token, correlationId) {
        ...
        if (!parsed)
            return ACCEPTED;
        await this.verificationEngine.verify({ ...parsed, expectedPurpose: client_1.VerificationPurpose.WHATSAPP_OTP, correlationId, afterVerified: async (verification, transaction) => {
                await this.repository.createAuditEvent({ subjectUserId: verification.userId, action: 'identity.whatsapp_verification.verified', metadata: { verificationId: verification.id, correlationId: correlationId ?? null } }, transaction);
...
exports.WhatsAppVerificationService = WhatsAppVerificationService = __decorate([
